...
from urdfpy import (rpy_to_matrix, matrix_to_rpy, matrix_to_xyz_rpy,
                    xyz_rpy_to_matrix)
from urdfpy.utils import rotation_to_affine, rotation_translation_to_affine
...
def test_rotation_to_affine():
    mat = torch.eye(3, dtype=torch.float32)
    affine = rotation_to_affine(mat)
    assert affine.equal(torch.eye(4, dtype=torch.float32))


def test_rotation_translation_to_affine():
    rot = torch.eye(3, dtype=torch.float32)
    trans = torch.zeros((3, 1), dtype=torch.float32)
    affine = rotation_translation_to_affine(rot, trans)
    assert affine.equal(torch.eye(4, dtype=torch.float32))
