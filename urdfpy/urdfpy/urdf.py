...
from .utils import (parse_origin, unparse_origin, get_filename, load_meshes,
                    configure_origin, rotation_from_axis_angle_torch, rotation_to_affine,
                    rotation_translation_to_affine)
...
class Joint(URDFType):
    ...
    def get_child_pose_torch(self, cfg=None):
        """Computes the child pose relative to a parent pose for a given
        configuration value.
        ...
        """
        if cfg is None:
            return torch.from_numpy(self.origin).float()
        elif self.joint_type == 'fixed':
            return torch.from_numpy(self.origin).float()
        elif self.joint_type in ['revolute', 'continuous']:
            if cfg is None:
                cfg = torch.tensor(0.0, dtype=torch.float32)
            # cfg is the rotation angle
            R = rotation_to_affine(rotation_from_axis_angle_torch(axis=torch.from_numpy(self.axis).float(), angle=cfg))
            return torch.from_numpy(self.origin).float().matmul(R)
        elif self.joint_type == 'prismatic':
            if cfg is None:
                cfg = torch.tensor(0.0, dtype=torch.float32)
            rotation = torch.eye(3, dtype=torch.float32)
            translation = torch.from_numpy(self.axis).float() * cfg
            affine = rotation_translation_to_affine(rotation, translation.unsqueeze(-1))
            return torch.from_numpy(self.origin).float().matmul(affine)
        elif self.joint_type == 'planar':
            if cfg is None:
                cfg = torch.zeros(2, dtype=torch.float32)
            if cfg.size(0) != 2 or len(cfg.size()) > 1:
                raise ValueError(
                    '(2,) float configuration required for planar joints'
                )
            rotation = torch.eye(3, dtype=torch.float32)
            translation = torch.from_numpy(self.origin).float()[:3,:2].matmul(cfg.view(2, 1))
            affine = rotation_translation_to_affine(rotation, translation)
            return torch.from_numpy(self.origin).float().matmul(affine)
        elif self.joint_type == 'floating':
            if cfg is None:
                cfg = torch.zeros(6, dtype=torch.float32)
            else:
                cfg_numpy = configure_origin(cfg)
                if cfg_numpy is None:
                    raise ValueError('Invalid configuration for floating joint')
                cfg = torch.from_numpy(cfg_numpy).float()
            return torch.from_numpy(self.origin).float().matmul(cfg)
        else:
            raise ValueError('Invalid configuration')
    ...
    def get_child_poses_torch(self, cfg, n_cfgs):
        """Computes the child pose relative to a parent pose for a given set of
        configuration values.
        ...
        """
        if cfg is None:
            return torch.from_numpy(self.origin).float().repeat(n_cfgs, 1, 1)
        elif self.joint_type == 'fixed':
            return torch.from_numpy(self.origin).float().repeat(n_cfgs, 1, 1)
        elif self.joint_type in ['revolute', 'continuous']:
            if cfg is None:
                cfg = torch.zeros(n_cfgs)
            return torch.matmul(torch.from_numpy(self.origin).float(), self._rotation_matrices_torch(cfg, self.axis))
        elif self.joint_type == 'prismatic':
            if cfg is None:
                cfg = torch.zeros(n_cfgs)
            translation = torch.eye(4).repeat(n_cfgs, 1, 1)
            translation[:,:3,3] = self.axis * cfg[:,None]
            return torch.matmul(torch.from_numpy(self.origin).float(), translation)
        elif self.joint_type == 'planar':
            raise NotImplementedError()
        elif self.joint_type == 'floating':
            raise NotImplementedError()
    ...
    def _rotation_matrices_torch(self, angles, axis):
        """Compute rotation matrices from angle/axis representations.
        ...
        rots : (n,4,4)
            The rotation matrices
        """
        axis = axis / torch.norm(torch.from_numpy(axis))
        ...
        M[:,2,2] = cosa
        M[:,:3,:3] += (
            torch.outer(axis, axis).repeat(len(angles), 1, 1) *
            (1.0 - cosa)[:, None, None]
        )
        M[:,:3,:3] += torch.Tensor([
            [0.0, -axis[2], axis[1]],
            [axis[2], 0.0, -axis[0]],
            [-axis[1], axis[0], 0.0]]
        ).repeat(len(angles), 1, 1) * sina[:, None, None]
        return M
...
class URDF(URDFType):
    ...
    def link_fk_torch(self, cfg=None, link=None, links=None, use_names=False):
        """Computes the poses of the URDF's links via forward kinematics.
        ...
        """
        # Process config value
        joint_cfg = self._process_cfg_torch(cfg)
        ...
        for lnk in self._reverse_topo:
            ...
            for i in range(len(path) - 1):
                ...
                if joint.mimic is not None:
                    ...
                    cfg = joint_cfg[joint]
                pose = joint.get_child_pose_torch(cfg).matmul(pose)

                # Check existing FK to see if we can exit early
                if parent in fk:
                    pose = fk[parent].matmul(pose)
                    break
            fk[lnk] = pose
    ...
    def link_fk_batch_torch(self, cfgs=None, link=None, links=None, use_names=False):
        """Computes the poses of the URDF's links via forward kinematics in a batch.
        ...
        fk : dict or (n,4,4) float
            ...
            nx4x4 matrix if ``link`` is specified.
        """
        joint_cfgs, n_cfgs = self._process_cfgs_torch(cfgs)
        ...
        for lnk in self._reverse_topo:
            ...
            for i in range(len(path) - 1):
                ...
                if joint.mimic is not None:
                    ...
                    cfg_vals = joint_cfgs[joint]
                poses = torch.matmul(joint.get_child_poses_torch(cfg_vals, n_cfgs), poses)
    ...
    def _process_cfg_torch(self, cfg):
        """Process a joint configuration spec into a dictionary mapping
        joints to configuration values.
        """
        joint_cfg = {}
        if cfg is None:
            return joint_cfg
        elif isinstance(cfg, torch.Tensor):
            if len(cfg) != len(self.actuated_joints):
                raise ValueError('Cfg must have same length as actuated joints '
                                 'if specified as a numerical array')
            for joint, value in zip(self.actuated_joints, cfg):
    ...
    def _process_cfgs_torch(self, cfgs):
        """Process a list of joint configurations into a dictionary mapping joints to
        configuration values.
        ...
        if isinstance(cfgs, torch.Tensor):
            ...
            if isinstance(cfgs[0], dict):
                ...
                cfgs = torch.as_tensor(cfgs, dtype=torch.float32)
                for i, j in enumerate(self.actuated_joints):
                    joint_cfg[j] = cfgs[:,i]
        else:
            raise ValueError('Incorrectly formatted config array')
