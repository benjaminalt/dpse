...
def resolve_package_url(filepath):
    from rospkg import RosPack
    package_expr = re.findall(r"package://.*?/", filepath)[0]
    package_name = package_expr.split("/")[-2]
    package_path = RosPack().get_path(package_name)
    filepath_relative_to_package = filepath.split(package_expr)[-1]
    return os.path.join(package_path, filepath_relative_to_package)
...
def load_meshes(filename):
    ...
    if "package://" in filename:
        filename = resolve_package_url(filename)
...
def rotation_from_axis_angle_torch(axis: torch.Tensor, angle: torch.Tensor) -> torch.Tensor:
    ct = torch.cos(angle)
    st = torch.sin(angle)
...
def rotation_to_affine(rotation_matrix: torch.Tensor) -> torch.Tensor:
    upper_block = torch.cat((rotation_matrix, torch.zeros((3, 1), dtype=rotation_matrix.dtype)), dim=-1)
    lower_block = torch.cat((torch.zeros((1, 3), dtype=rotation_matrix.dtype),
                             torch.ones((1, 1), dtype=rotation_matrix.dtype)), dim=-1)
    return torch.cat((upper_block, lower_block), dim=0)


def rotation_translation_to_affine(rotation_matrix: torch.Tensor, translation_vector: torch.Tensor) -> torch.Tensor:
    upper_block = torch.cat((rotation_matrix, translation_vector), dim=-1)
    lower_block = torch.cat((torch.zeros((1, 3), dtype=rotation_matrix.dtype),
                             torch.ones((1, 1), dtype=rotation_matrix.dtype)), dim=-1)
    return torch.cat((upper_block, lower_block), dim=0)


def show_joint_trajectory(urdf, joint_trajectory):
    plt.show()
    fig, ax = plt.subplots()
    for i in range(joint_trajectory.size(-1)):
        ax.plot(range(joint_trajectory.size(0)), joint_trajectory[:, i], label=str(i + 1))
        ax.legend()
    plt.show()

    urdf.animate(joint_trajectory.numpy())
